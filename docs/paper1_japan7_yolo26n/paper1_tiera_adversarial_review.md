...
4. **BiFPN used a large unrelated dependency surface and ambiguous parser semantics.** `tasks.py` imported the entire `v9.py`, which transitively loaded packages such as pandas and IPython. The replacement is a minimal weighted-concat module; the parser injects the input count explicitly, validates shape/count, and initializes to ordinary concatenation.
...
- PyTorch `.pt` checkpoints are trusted binary artifacts. Path confinement prevents reading an arbitrary external file, but it does not make a hostile pickle safe; only project-controlled checkpoints should be passed to `--pretrained`.
