...
The A2 1e smoke completed with AMP, full `708/708` transfer, and all required artifacts. Its real first-batch weighted penalty was `3.1%-3.5%` of CIoU and its gradient norm was `10.4%-11.4%` of the CIoU gradient, so A2 was active without dominating localization. The full 2,119-image/5,000-GT validation assignment diagnostic found that D10 had zero zero-positive GTs and `9.956` one2many positives per GT; B1 assigner modification is therefore not supported. D10 median candidate CIoU was only `0.510` versus `0.774` overall, falling to `0.536` for AR 5-8 and `0.408` for AR at least 8. This is direct evidence for an elongated-box localization problem that A2 can reach, so A2 is authorized for one matched 30e signal. Classification scores remain globally immature after one epoch; defer B2 until the 30e result. Reports: `paper1_a2_assignment_summary.md` and `.json`.
