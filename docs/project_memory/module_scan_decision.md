...
The corrected audit passes the same 246 explicit checks under both normal Python and `python -O`. It covers exact full-criterion and per-parameter-gradient recovery at weight zero, both end-to-end branches, malformed input rejection, mixed FP16/FP32 AMP input, extreme aspect ratios, empty-target backward, baseline-identical architecture, `708/708` transfer, and the trusted checkpoint hash. The saved reports are `experiments/module_scan/paper1_a2_adversarial_audit.md` and `.json`; DDP is explicitly unverified because the formal protocol is single GPU. This authorizes only a 1e mechanical smoke. Before any 30e A2 run, diagnose per-GT positive counts, zero-positive rate, aspect-ratio bins, best candidate IoU, and class confidence. Use that evidence to choose A2, B1 assigner work, or B2 difficult-positive classification; do not alter the assigner beforehand. C remains unauthorized until A or B produces a positive matched signal.
