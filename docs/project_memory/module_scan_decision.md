...
The dataset audit found one zero-width D20 training label (`Japan_Japan_001265.txt`, line 4) and one duplicated validation label. Both were corrected in the source dataset, then Japan7 was rebuilt from scratch. The clean derived dataset has `19,752` training boxes and `5,000` validation boxes with zero missing labels, duplicates, or invalid lines.

P2/P2Lite, generic detail blocks, LaplacianConv, and attention scanning are paused. A1 added a minimal selectable standard Shape-IoU path without using the duplicated `shapeloss.py`; its matched 30e result was `0.573/0.318` versus B0 `0.574/0.319`, while D10 AP50-95 fell from `0.130` to `0.124`. Retain A1 as a neutral method-selection comparison, but reject 100e and final-composite promotion.

A2 retains baseline CIoU and adds `0.1 * tanh(|delta log-aspect|) * tanh(|target log-aspect|)`. The adversarial correction removed the earlier side-length term because it duplicated scale/aspect behavior already present in CIoU. The added term is class-agnostic, computed in a float32 log domain with dtype-aware width/height floors, and remains bounded. It only acts on `fg_mask` positives selected by the assigner; it cannot recover a GT that received no positive samples.

The corrected audit passes the same 251 explicit checks under both normal Python and `python -O`. It covers exact full-criterion and per-parameter-gradient recovery at weight zero, both end-to-end branches, malformed input rejection, mixed FP16/FP32 AMP input, extreme aspect ratios, empty-target backward, baseline-identical architecture, `708/708` transfer, and the trusted checkpoint hash. The saved reports are `experiments/module_scan/paper1_a2_adversarial_audit.md` and `.json`; DDP is explicitly unverified because the formal protocol is single GPU. An A2 smoke also saves one real assigned-positive decomposition for each E2E branch to `a2_penalty_diagnostics.json`. This authorizes only a 1e mechanical smoke. Before any 30e A2 run, diagnose per-GT positive counts, zero-positive rate, aspect-ratio bins, best candidate IoU, and class confidence. Use that evidence to choose A2, B1 assigner work, or B2 difficult-positive classification; do not alter the assigner beforehand. C remains unauthorized until A or B produces a positive matched signal.

The A2 1e smoke completed with AMP, full `708/708` transfer, and all required artifacts. Its real first-batch weighted penalty was `3.1%-3.5%` of CIoU and its gradient norm was `10.4%-11.4%` of the CIoU gradient, so A2 was active without dominating localization. The full 2,119-image/5,000-GT validation assignment diagnostic found that D10 had zero zero-positive GTs and `9.956` one2many positives per GT; B1 assigner modification is therefore not supported.

A2 then completed its matched 30e signal at `0.574/0.319`, exactly tying B0 overall while D10 fell from `0.324/0.130` to `0.305/0.121` AP50/AP50-95. Matched best-checkpoint diagnostics showed D10 candidate CIoU `0.8169 -> 0.8158` and class score `0.1779 -> 0.1755` from B0 to A2, confirming that the bounded shape term did not improve its intended mechanism. Reject A2 from 100e, weight sweeps, and composites. B1 remains rejected because positive assignment is already complete. Authorize only B2 design/audit next: bounded reweighting of high-quality matched positives with low classification confidence, defined without class IDs. C remains unauthorized until B2 has a positive matched signal.
...
Remote RTX 4090 adversarial audits passed `65/65` in both normal Python and `python -O`, including trusted checkpoint SHA256, bitwise `708/708` transfer, CUDA AMP, mixed and empty targets, bounded weights, and finite gradients. The 1e smoke passed all artifact gates and its real added/base gradient ratios were `0.1821` in both E2E branches.

The matched B2 30e signal at commit `9f7bb9d` reached `0.571/0.317` mAP50/mAP50-95 versus B0 `0.574/0.319`; D10 AP50/AP50-95 dropped `0.324/0.130 -> 0.297/0.117`. B2 is rejected from 100e, lambda sweeps, and composites. A1/A2/B1/B2 are now closed. The next authorized hypothesis must be a single, lightweight direction-aware feature representation candidate, with a fresh source audit and smoke before a 30e signal.
