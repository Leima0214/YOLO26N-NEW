...
A2 retains baseline CIoU and adds a configurable bounded elongation penalty. It is class-agnostic, weights targets by geometry rather than D10 identity, and is capped so thin boxes cannot create an unbounded added term. Its mandatory audit covers exact lambda-zero loss/gradient recovery, orientation symmetry, square and extremely thin boxes, empty-target batches, finite gradients, CUDA FP16, baseline-identical architecture, and `708/708` checkpoint transfer. Only after A2 is screened may work proceed to a diagnostic of per-GT positive counts and confidence; do not modify the assigner before that evidence selects B1 or B2. C remains unauthorized until A or B produces a positive matched signal.
