...
class YOLODataset(BaseDataset):
    ...
    def _replace_path_dir(self, path: str | Path, src_dir: str, dst_dir: str) -> Path:
        """Replace the last matching directory name in a path."""
        p = Path(path)
        parts = list(p.parts)
        for i in range(len(parts) - 2, -1, -1):
            if parts[i].lower() == src_dir.lower():
                parts[i] = dst_dir
                return Path(*parts)
        return p.parent.parent / dst_dir / p.name

    def _rgbir_pair_path(self, rgb_path: str | Path, target_dir: str) -> Path:
        """Map a visible image path to its paired IR image or label path."""
        rgb_dir = self.data.get("rgbir_rgb_dir", "vi")
        path = self._replace_path_dir(rgb_path, rgb_dir, target_dir)
        if path.exists():
            return path
        matches = sorted(path.parent.glob(f"{path.stem}.*"))
        return matches[0] if matches else path

    def _rgbir_label_paths(self, img_paths: list[str]) -> list[str]:
        """Map original M3FD visible image paths to the sibling labels directory."""
        label_dir = self.data.get("rgbir_label_dir", "labels")
        return [str(self._rgbir_pair_path(p, label_dir).with_suffix(".txt")) for p in img_paths]

    def read_image(self, f: str) -> np.ndarray | None:
        """Read RGB and IR original M3FD files as one [R, G, B, IR] array when configured."""
        if not self.data.get("rgbir_pair", False):
            return super().read_image(f)

        rgb_bgr = super().read_image(f)
        if rgb_bgr is None:
            return None
        if rgb_bgr.ndim == 2:
            rgb_bgr = rgb_bgr[..., None]
        if rgb_bgr.shape[2] == 1:
            rgb = np.repeat(rgb_bgr, 3, axis=2)
        else:
            rgb = cv2.cvtColor(rgb_bgr[..., :3], cv2.COLOR_BGR2RGB)

        ir_path = self._rgbir_pair_path(f, self.data.get("rgbir_ir_dir", "ir"))
        ir = cv2.imdecode(np.fromfile(str(ir_path), dtype=np.uint8), cv2.IMREAD_GRAYSCALE)
        if ir is None:
            raise FileNotFoundError(f"IR image not found for RGB image {f}: {ir_path}")
        if ir.shape[:2] != rgb.shape[:2]:
            ir = cv2.resize(ir, (rgb.shape[1], rgb.shape[0]), interpolation=cv2.INTER_LINEAR)
        return np.concatenate((rgb, ir[..., None]), axis=2)
    ...
    def get_labels(self) -> list[dict]:
        ...
        """
        self.label_files = self._rgbir_label_paths(self.im_files) if self.data.get("rgbir_pair", False) else img2label_paths(self.im_files)
