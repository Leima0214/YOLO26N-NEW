...
class PAMBConv(nn.Module):
    def __init__(self, c1, c2, k=3, s=1, expansion=1, se_ration=0, dropout_rate=0.2, drop_connect_rate=0.2):
        super(PAMBConv, self).__init__()
        self.pa = PatchExpand(dim=c1, dim_scale=2)
        self.pa_out_channels = c1 // self.pa.dim_scale

        self.has_shortcut = False
        in_ch = self.pa_out_channels

        expanded_c = in_ch * expansion
        self.expansion_conv = stem(in_ch, expanded_c, kernel_size=1, stride=1)
