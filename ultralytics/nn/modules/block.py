...
class SAMAuxBranchFusion(nn.Module):
    """YOLO main branch + SAM-style auxiliary branch fusion block."""
    ...
    def __init__(
        ...
        """Initialize dual-branch fusion block."""
        super().__init__()
        c_aux = max(32, int(c2 * e))
        self.main = Conv(c1, c2, k=1, s=1)
        self.aux_pre = Conv(c1, c_aux, k=1, s=1)
        self.aux_spatial = DWConv(c_aux, c_aux, k=k, s=1, act=True)
        self.aux_post = Conv(c_aux, c2, k=1, s=1, act=False)
        ...
        self.out = Conv(c2, c2, k=3, s=1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Fuse YOLO main features with SAM-style auxiliary features."""
        y_main = self.main(x)
        y_aux = self.aux_post(self.aux_spatial(self.aux_pre(x)))
        if self.use_gate:
            y_aux = y_aux * self.gate(y_aux)
        return self.out(y_main + y_aux)


class SAMAdapterInject(nn.Module):
    """Lightweight adapter that injects SAM-style visual priors into YOLO features."""
    ...
    def __init__(
        ...
        """Initialize SAM adapter injection block."""
        super().__init__()
        self.base = Conv(c1, c2, k=1, s=1) if c1 != c2 else nn.Identity()
        c_hidden = max(16, int(c2 * r))
        self.down = Conv(c2, c_hidden, k=1, s=1)
        self.spatial = DWConv(c_hidden, c_hidden, k=k, s=1, act=True)
        self.up = Conv(c_hidden, c2, k=1, s=1, act=False)
        self.alpha = nn.Parameter(torch.zeros(1))
        self.refine = Conv(c2, c2, k=3, s=1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Inject adapter residual into YOLO feature stream."""
        y = self.base(x)
        delta = self.up(self.spatial(self.down(y)))
        return self.refine(y + self.alpha * delta)
